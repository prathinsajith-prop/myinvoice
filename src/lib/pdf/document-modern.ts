...
export async function generateModernDocumentPdf(data: DocumentPdfData): Promise<Uint8Array> {
    ...
    if (isDelivery) {
        ["Description", "Qty", "Unit", "Notes"].forEach(([h, x]: [string, number]) => page.drawText(h, { x, y, size: 8.5, font: bold, color: rgb(0.3, 0.3, 0.3) }));
        // Correction: use simple approach
    ...
    if (!isDelivery && data.subtotal !== undefined) {
        ...
        page.drawText(money(data.subtotal, data.currency), { x: 460, y, size: 9, font }); y -= 14;
        if (data.totalDiscount && data.totalDiscount > 0) {
            page.drawText("Discount", { x: 342, y, size: 9, font, color: rgb(0.1, 0.5, 0.1) });
            page.drawText(`-${money(data.totalDiscount, data.currency)}`, { x: 460, y, size: 9, font, color: rgb(0.1, 0.5, 0.1) }); y -= 14;
        }
        page.drawText("VAT", { x: 342, y, size: 9, font, color: rgb(0.45, 0.45, 0.45) });
        page.drawText(money(data.totalVat ?? 0, data.currency), { x: 460, y, size: 9, font }); y -= 14;
        page.drawText("Total", { x: 342, y, size: 10.5, font: bold, color: rgb(ar, ag, ab) });
        page.drawText(money(data.total ?? 0, data.currency), { x: 460, y, size: 10.5, font: bold, color: rgb(ar, ag, ab) }); y -= 14;
    ...
    if (data.notes && y > 80) { y -= 14; page.drawText("Notes", { x: margin, y, size: 9, font: bold }); y -= 12; page.drawText(data.notes.slice(0, 280), { x: margin, y, size: 8.5, font, color: rgb(0.4, 0.4, 0.4) }); }
    if (data.terms && y > 80) { y -= 12; page.drawText("Terms & Conditions", { x: margin, y, size: 9, font: bold }); y -= 12; page.drawText(data.terms.slice(0, 280), { x: margin, y, size: 8.5, font, color: rgb(0.4, 0.4, 0.4) }); }
