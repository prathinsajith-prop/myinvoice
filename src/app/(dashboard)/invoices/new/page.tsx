...
function calcLine(qty: number, unitPrice: number, discountPct: number, vatTreatment: string) {
    const subtotal = qty * unitPrice;
    const discountAmt = subtotal * (discountPct / 100);
    const taxable = subtotal - discountAmt;
    const vatAmt = taxable * (VAT_RATES[vatTreatment] ?? 0.05);
    const lineTotal = taxable + vatAmt;
    return { subtotal, discountAmt, taxable, vatAmt, lineTotal };
}
...
export default function NewInvoicePage() {
    ...
    const totals = watchedItems.reduce(
        (acc, item) => {
            const { subtotal, discountAmt, taxable, vatAmt, lineTotal } = calcLine(
                Number(item.quantity) || 0,
                Number(item.unitPrice) || 0,
                Number(item.discountPercent) || 0,
                item.vatTreatment ?? "STANDARD",
            );
            return {
    ...
    function applyProduct(index: number, productId: string) {
        ...
        form.setValue(`lineItems.${index}.vatTreatment`, product.vatTreatment ?? "STANDARD");
    }
                ...
                <Card>
                    <CardHeader className="flex flex-row items-center justify-between">
                        ...
                        <Button
                            ...
                            size="sm"
                            onClick={() =>
                                append({ description: "", quantity: 1, unitPrice: 0, discountPercent: 0, vatTreatment: "STANDARD" })
                            }
                        >
                    ...
                    <CardContent className="space-y-4">
                        {form.formState.errors.lineItems?.root && (
                            <p className="text-xs text-destructive">{form.formState.errors.lineItems.root.message}</p>
                        )}
                                    ...
                                    <div className="grid gap-3 sm:grid-cols-2">
                                        ...
                                        <div className="sm:col-span-2 space-y-1.5">
                                            ...
                                            {form.formState.errors.lineItems?.[index]?.description && (
                                                <p className="text-xs text-destructive">{form.formState.errors.lineItems[index]?.description?.message}</p>
                                            )}
                                        </div>
                ...
                <div className="grid gap-6 lg:grid-cols-2">
                    ...
                    <Card>
                        ...
                        <CardContent className="space-y-4">
                            <div className="space-y-1.5">
                                <Label>Notes</Label>
                                <Textarea placeholder="Additional notes for the customer..." rows={3} {...form.register("notes")} />
                            </div>
                            <div className="space-y-1.5">
                    ...
                    <Card>
                        ...
                        <CardContent className="space-y-3">
                            ...
                            <div className="flex justify-between text-sm">
                                ...
                                <span>AED {totals.taxable.toFixed(2)}</span>
                            </div>
                            <div className="flex justify-between text-sm">
